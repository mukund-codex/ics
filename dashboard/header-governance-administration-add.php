<?php
	...
	$pageName = "Governance & Administration";
	$parentPageURL = 'headercontent-master.php';
	$pageURL = 'header-governance-administration-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addGovernanceAdministration($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueGovernanceAdministrationById($id,$category);
	}
	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
            ...
            $category = $admin->escape_string($admin->strip_all($_GET['category']));
			$result = $admin->updateGovernanceAdministration($_POST ,$_FILES,$category);
			
			header("location:".$pageURL."?updatesuccess&edit&category=".$_GET['category']."&id=".$id);
			exit;
		}
	}
?>
            ...
            <form role="form" action="" method="post" id="form" enctype="multipart/form-data">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h6 class="panel-title"><i class="icon-library"></i>Add About Sanstha</h6>
                    </div>
                    <div class="panel-body">
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <label for="sel1">Select Governance & Administration</label>
                                    <?php  
                                        if(isset($_GET['edit']) && isset($_GET['category'])){
                                    ?>
                                    <select class="form-control" id="gover_admin" name="gover_admin" disabled>
                                       
                                        <option value="<?php echo $_GET['category'];?>" selected ><?php echo $_GET['category'];?></option>
                                    </select>
                                    <?php } else{ ?>
                                        <select class="form-control" id="gover_admin" name="gover_admin">
                                        <option value="0">Select Governance & Administration</option>
                                        ...
                                        <option value="College Devlopment Committee">College Devlopment Committee</option>
                                        <option value="Academics Council">Academics Council</option>
                                    </select>
                                    <?php } ?>
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
									...
									?>
                                    <embed src="../img/headers/<?php echo $file_name.'.'.$ext ?>" width="400px"
                                        height="330px" />
                                    <?php
									}  }?>
                                </div>
