<?php
	...
	$pageName = "Package Master";
	$parentPageURL = 'package-master.php';
	$pageURL = 'package-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$category_name = trim($admin->escape_string($admin->strip_all($_POST['category_name'])));
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniquePackageById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$id 			= trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$package_name 	= trim($admin->escape_string($admin->strip_all($_POST['package_name'])));
			if(empty($id) || empty($package_name)){
				header("location:".$pageURL."?updatefail&msg=Please enter a category_name&edit&id=".$id);
				exit();
			}
			else {
				//update to database
				$result = $admin->updatePackageMaster($_POST,$_FILES);
			...
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i>Package Details</h6>
					</div>
						...
						<div class="form-group">
							<div class="row">
								<div class="col-sm-4">
									...
									<input type="text" class="form-control" id ="package_name" name="package_name" value="<?php if(isset($_GET['edit'])){ echo $data['package_name']; }  ?>" >			
							    </div>
								<div class="col-sm-4">
