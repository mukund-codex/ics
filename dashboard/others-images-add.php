<?php
	...
	$pageName = "Other Images Courses";
	$parentPageURL = 'welcome.php';
	$pageURL = 'others-images-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addOtherImages($_POST,$_FILES,'Other');
            // print_r($_FILES);exit();
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueOtherImagesById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateOtherImages($_POST,$_FILES);
							...
							<div class="row">
								...
								<div class="col-sm-4">
									...
									<?php if(isset($_GET['edit'])) {
										$file_name = str_replace('', '-', strtolower( pathinfo($data['other_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($data['other_image'], PATHINFO_EXTENSION);
									?>
										<img src="../img/Other/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  />
									<?php
									} ?>
								</div> 	 
								<div class="col-sm-4">
