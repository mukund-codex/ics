<?php
	...
	class AdminFunctions extends Database {
        ...
        function addDepartmentData($data, $file) {
			...
			$query = "insert into ".PREFIX."departments (department_id, image, text, image_text, pdf,category_type, created_at) values($department_id, '$image_name', '$text', '$image_text', '$image_name_pdf','$category', '$date') ";
			 //echo $query;exit();
		...
		function updateDepartmentData($data, $file) {
			...
			$query = "update ".PREFIX."departments set image = '$image_name' ,text='$text', image_text ='$image_text', pdf='$image_name_pdf',category_type='$category' ,updated_at='$date' where id = ".$id."";
			// echo $query;exit();
			return $this->query($query);
		}
		...
		function addDepartmentDataImages($data,$file){
			...
			$query = "insert into ".PREFIX."departments_images (category, image, dept_title, dept_discp, created_at) values('$category' , '$image_name', '$dept_title', '$dept_description', '$date') ";
			 //echo $query;exit();
			return $this->query($query);
		}
		...
		//	print_r($data);exit();
			...
			if(isset($file['image']['name']) && !empty($file['image']['name'])) {
				...
				$this->unlinkImage("", $Detail['image'], "crop");
				$image_name = $SaveImage->uploadCroppedImageFileFromForm($file['image'], 1366, $cropData, $imgDir, $file_name.'-'.time().'-1');
				$this->query("update ".PREFIX."departments_images set image ='$image_name' ,dept_discp = '$dept_description',  dept_title = '$dept_title' where id='$id' ");
			}
       ...
       //deapartment image upload add
		...
		function deleteDepartmentData($id) {
			...
			$query = "delete from ".PREFIX."departments where id='$id'";
			$this->query($query);
			return true;
		}
		...
		/**
		 * UndergraduateCourses Page
		 */
		function addUndergraduateCoursesDataImages($data,$file){
            $date = date("Y-m-d H:i:s");
		...
		function getUniqueUndergraduateCoursesDataImagesById($id){
			$id = $this->escape_string($this->strip_all($id));
		...
		function updateUndergraduateCoursesDataImages($data,$file){
			$date = date("Y-m-d H:i:s");
			$id = $this->escape_string($this->strip_all($data['id']));
		...
		function deleteUndergraduateCoursesData($id) {
			$id = $this->escape_string($this->strip_all($id));
		...
		function updateBanner($data,$file) {
			...
			if(isset($file['banner_img']['name']) && !empty($file['banner_img']['name'])) {
				...
				$image_name = $SaveImage->uploadCroppedImageFileFromForm($file['banner_img'], 1366, $cropData, $imgDir, $file_name.'-'.time().'-1');
				$this->query("update ".PREFIX."banner_master set title='$title',sub_title='$sub_title', display_order='$display_order' where id='$id'");
			}
		...
		function addUndergraduateImages($data,$file){
			...
			$query = "insert into ".PREFIX."undergraduate_images(title,sub_title,undergraduate_image,display_order,created) values ('$title','$sub_title','$image_name', '$display_order','$date')";

			$category = "insert into ".PREFIX."category(category,sub_category) values ('$title','UG')";
		...
		function addPostgraduateImages($data,$file){
			...
			$query = "insert into ".PREFIX."postgraduate_images(title,sub_title,postgraduate_image,display_order,created) values ('$title','$sub_title','$image_name', '$display_order','$date')";

			$category = "insert into ".PREFIX."category(category,sub_category) values ('$title','PG')";
		...
		function addInfrastructureImages($data,$file){
			...
			$query = "insert into ".PREFIX."infrastructure_images(title,sub_title,Infra_image,display_order,created) values ('$title','$sub_title','$image_name', '$display_order','$date')";
		...
		function updatePostgraduateImages($data,$file) {
			...
			if(isset($file['image_name']['name']) && !empty($file['image_name']['name'])){
				...
				$cropData = $this->strip_all($data['cropData1']);
				$image_name = $SaveImage->uploadCroppedImageFileFromForm($file['image_name'], 800 , $cropData, $imgDir, $file_name.'-'.time().'-1');
				$this->query("update ".PREFIX."postgraduate_images set image_name='".$image_name."' where id='".$id."'");
			} 
		...
		function updateInfrastructureImages($data,$file) {
			...
			if(isset($file['image_name']['name']) && !empty($file['image_name']['name'])){
				...
				$cropData = $this->strip_all($data['cropData1']);
				$image_name = $SaveImage->uploadCroppedImageFileFromForm($file['image_name'], 1440, $cropData, $imgDir, $file_name.'-'.time().'-1');
				$this->query("update ".PREFIX."infrastructure_images set image_name='".$image_name."' where id='".$id."'");
			} 
