<?php
	...
	$pageName = "Undergraduate";
	$parentPageURL = 'welcome.php';
	$pageURL = 'undergraduate-images-add.php';
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueUndergraduateImagesById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateUndergraduateImages($_POST,$_FILES);
							...
							<div class="row">
								...
								<div class="col-sm-4">
									...
									<?php if(isset($_GET['edit'])) {
										$file_name = str_replace('', '-', strtolower( pathinfo($data['undergraduate_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($data['undergraduate_image'], PATHINFO_EXTENSION);
									?>
										<img src="../img/Undergraduate/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  />
									<?php
									} ?>
								</div> 	 
								<div class="col-sm-4">
