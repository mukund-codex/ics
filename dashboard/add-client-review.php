<?php
	...
	$pageName = "Client Review";
	$parentPageURL = 'gallery-review-master.php';
	$pageURL = 'add-client-review.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addClientReview($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueClientReviewById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateClientReview($_POST,$_FILES);
						...
						<div class="form-group">
							<div class="row">
								...
								<div class="col-sm-4">
									...
									<span class="help-text">
										...
										Images must be exactly <strong>200x199</strong> pixels.
									</span>
									<br>
								...
								<div class="col-sm-4">
									...
									<input type="text" class="form-control"  placeholder="Name"  name="name" id="" value="<?php if(isset($_GET['edit'])){ echo $data['name']; }?>"/>
								</div>								
								<div class="col-sm-4">
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<textarea col="5" rows="4"  class="form-control" required  name="review" id="" /><?php if(!empty($data['review'])){ echo $data['review']; }?></textarea>
								</div>
...
<?php 	include "include/footer.php"; ?>
	...
	<script type="text/javascript">
		/* $("form").submit( function(e) {
           var review = CKEDITOR.instances['review'].getData().replace(/<[^>]*>/gi, '').length;
           if( !review ) {
