<?php
	...
	$pageName = "Postgraduate";
	$parentPageURL = 'welcome.php';
	$pageURL = 'postgraduate-images-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addPostgraduateImages($_POST,$_FILES,'PGC');
            // print_r($_FILES);exit();
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniquePostgraduateImagesById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updatePostgraduateImages($_POST,$_FILES);
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="file" id="img" class="form-control"
                                        accept="image/jpeg,image/png,image/jpg" name="image_name"
                                    ...
                                    <?php if(isset($_GET['edit'])) {
										$file_name = str_replace('', '-', strtolower( pathinfo($data['postgraduate_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($data['postgraduate_image'], PATHINFO_EXTENSION);
									?>
                                    <img src="../img/Postgraduate/<?php echo $file_name.'_crop.'.$ext ?>" width="100" />
                                    <?php
									} ?>
                                </div>
                                <div class="col-sm-4">
                                    <label>Title</label>
                                    <textarea col="5" rows="4"  class="form-control" required  
                                    name="title" id="" /><?php if(isset($_GET['edit'])){ echo $data['title']; }?></textarea>

                                </div>
                                <div class="col-sm-4">
                                    <label>Sub Title</label>
                                    <textarea col="5" rows="4"  class="form-control" required 
                                     name="sub_title" id="" /><?php if(isset($_GET['edit'])){ echo $data['sub_title']; }?></textarea>
                            ...
                            <div class="row">
                                ...
                                <!-- <div class="col-sm-4">
                                    ...
                                    <input type="text" class="form-control" min="1" placeholder="Display Order"
                                        name="display_order" id=""
                                        value="<?php if(isset($_GET['edit'])){ echo $data['display_order']; }?>" />
                                </div> -->
                            </div>
                        </div>
                    </div>
