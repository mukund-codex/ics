<?php
	...
	$pageName = "Department Images";
	$parentPageURL = 'department-master.php';
	$pageURL = 'department_image-add.php';
    // echo $_GET['id'];exit();
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addDepartmentDataImages($_POST,$_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueDepartmentDataImagesById($id);
	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			$result = $admin->updateDepartmentDataImages($_POST,$_FILES);
...
<body class="sidebar-wide">
    ...
    var editor = CKEDITOR.replace('dept_title', {
        height: 200,
        filebrowserImageBrowseUrl: 'js/editor/ckfinder/ckfinder.html?type=Images',
        filebrowserImageUploadUrl: 'js/editor/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Images',
        toolbarGroups: [
