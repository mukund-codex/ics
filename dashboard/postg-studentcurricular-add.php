<?php
	...
	$pageName = "Student Co-Curricular Activities";
	$parentPageURL = 'postgraduate-courses-master.php';
	$pageURL = 'postg-studentcurricular-add.php';
   ...
   $removeSelectedtabs = $admin->removeSelectedTabs();
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addStudentStudentCurricular($_POST,$_FILES,$type);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueStudentCurricularById($id , $type);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateStudentCurricular($_POST,$_FILES,$type);
			...
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i>Student Co-Curricular</h6>
					</div>
						...
						<div class="row">
							...
							<div class="row">
								<div class="col-sm-6">
									...
									<input type="file" class="form-control" name="stdcirrcular_img"  accept="image/jpg,image/png,image/jpeg" <?php if(!isset($_GET['edit'])) { echo 'required'; } ?> id="" data-image-index="0" />
									...
									?>
										Previous Image : <br><img src="../img/StudentCo-Curricular/postgraduate/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  /><br><br>
									<?php
									} ?>
								</div>
