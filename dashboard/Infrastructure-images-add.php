<?php
	...
	$pageURL = 'Infrastructure-images-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addInfrastructureImages($_POST,$_FILES);
            // print_r($_FILES);exit();
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getInfrastructureById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg','image/png');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateInfrastructureImages($_POST,$_FILES);
							...
							<div class="row">
								...
								<div class="col-sm-4">
									...
									<span class="help-text">
										...
										Images must be exactly <strong>1440x900</strong> pixels.
									</span>
									<br>
									<?php if(isset($_GET['edit'])) {
										$file_name = str_replace('', '-', strtolower( pathinfo($data['Infra_image'], PATHINFO_FILENAME)));
										$ext = pathinfo($data['Infra_image'], PATHINFO_EXTENSION);
									?>
										<img src="../img/Infrastructure/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  />
									<?php
									} ?>
								</div> 	 
								<div class="col-sm-4">
...
<?php 	include "include/footer.php"; ?>
	...
	$(document).ready(function() {
		$('#img').change(function(){
			...
			loadImagePreview(this, (1440 / 900));
		});
	});
	</script>
</body>
