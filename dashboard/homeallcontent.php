<?php
	...
	$pageName = "Home Content Only Title Change";
	$pageURL = 'homeallcontent.php';
	...
	if(isset($_POST['update'])) {
        ...
        // print_r($_POST);exit;
		$result = $admin->homeAllTitleChange($_POST);
	...
	$qry = $admin->query("SELECT * FROM ".PREFIX."home_all_title_changes");
			...
			<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="form-group">
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="form-group" id="hometitle">
                                        <label for="sel1">Select Home Content Title to Change:</label>
                                        <select class="form-control" id="home" name="home">
                                            <option value="0">Select Home Content Title to Change</option>
                                            <?php
					
							                    while($row = $admin->fetch($qry)){
								
                                            ?>
                                            <option value="<?php echo $row['content_id'] ?>"><?php echo $row['content'] ?></option>
                                            <?php } ?>
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<textarea col="5" rows="4"  class="form-control" required  name="chgtitle" id="chgtitle" /></textarea>
								</div>
...
<?php 	include "include/footer.php"; ?>
	...
	<script type="text/javascript">
		...
		var editor = CKEDITOR.replace( 'chgtitle', {
			height: 200,
    ...
    $(function() {
        $("#hometitle").click(function() {
           ...
           // var courses_name = $('select[name=home] option:selected').text();
            var content_id = $('select[name=home] option:selected').val();
            $('#content_id').val(content_id);
            // var courses_subtype = $('#courses_subtype').val();
            
            $.ajax({
                    type: "POST",
                    url: "ajaxhometitleonlychange.php",
                    data: {"content_id" : content_id},
                
                    success: function(response) {
                       // console.log(response)
                       
                       CKEDITOR.instances['chgtitle'].setData(response)
                        //$('#chgtitle').val(response);
