<?php
	...
	$pageName = "Courses SubTypes";
	$parentPageURL = 'postgraduate-courses-master.php';
	$pageURL = 'postg-subtype-courses-images-add.php';
   ...
   $getAlltabs = $admin->getAllBoxesTabs();
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			...
			$result = $admin->addCourseSubtype($_POST,$_FILES,$type);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueCourseSubtypeById($id);
	}
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			$allowed_ext = array('image/jpeg','image/jpg');
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateCourseSubtype($_POST,$_FILES);
			...
			<form role="form" action="" method="post" id="form" enctype="multipart/form-data">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h6 class="panel-title"><i class="icon-library"></i> Courses SubType Details</h6>
					</div>
                        ...
                        <div class="row">
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<input type="file" class="form-control" name="coursesubtype_img"  accept="image/jpg,image/png,image/jpeg" <?php if(!isset($_GET['edit'])) { echo 'required'; } ?> id="" data-image-index="0" />
									...
									<?php if(isset($_GET['edit'])) {
										$file_name = str_replace('', '-', strtolower( pathinfo($data['coursesubtype_img'], PATHINFO_FILENAME)));
										$ext = pathinfo($data['coursesubtype_img'], PATHINFO_EXTENSION);
									?>
										Previous Image : <br><img src="../img/coursesubtype/<?php echo $file_name.'_crop.'.$ext ?>" width="100"  /><br><br>
									<?php
									} ?>
								</div>
							...
							<div class="row">	
								...
								<div class="col-sm-6">
									<label>Sub Title Course Name</label>
									<textarea col="5" rows="4"  class="form-control" required  name="sub_title" id="" /><?php if(isset($_GET['edit'])){ echo $data['courses_subtype']; }?></textarea>
								</div>
